...
2026-09-27 FRP 路径已实装：选用正在运行的 VM104，安装独立 `nomad-frpc.service`；阿里云上海另建 `nomad-frps.service`，专用令牌与私有 CA 由受限服务器文件持有，客户端校验证书身份。VM104 出站 7001、云端仅网桥 `172.17.0.1:25245`→VM104 回环 43104；私有健康 200、匿名导入记录 401、NPM 容器可达，两个新服务分别重启后自动恢复，旧 Docker frps 7000/OpenList25244 未改。公网直连25245超时，Nomad 公网域名/可信 HTTPS 反代和正式 Origin/audience 仍未配置；当前只是私有隧道，不算原生 API 验收。详见 `ops/homelab-frp/README.md` 和 `evidence/story-1-8-homelab-2026-09-27/frp-acceptance.json`。Story1.8仍in-progress，双端真机、真实XHS和其他既有门槛不变。
