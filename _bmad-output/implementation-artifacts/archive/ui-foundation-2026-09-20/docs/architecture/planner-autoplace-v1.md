# Planner Autoplace v1 (Historical)

> Superseded by `planner-orchestration-v2.md` for current product architecture.
> This file is retained only to explain the delivered v0.4 seed/Quick baseline;
> its fixed 2h blocks, `selected_required`-only intent, quota and visible seed
> semantics are not current MVP requirements.
