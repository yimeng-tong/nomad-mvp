...
export async function assertImportRecordSchema(db: PrismaClient): Promise<void> {
  await authAuthority(async () => {
    await db.$queryRaw`SELECT id,"userId",job_id,normalized_url,normalization_version,original_url_protected,status FROM "ImportRecord" LIMIT 0`;
    const rows = await db.$queryRaw<Array<{ ok: boolean }>>`SELECT (
      ...
      AND (SELECT count(*) FROM pg_constraint WHERE conrelid='"ImportRecord"'::regclass AND convalidated
        AND conname IN ('ImportRecord_userId_fkey','ImportRecord_job_id_userId_fkey','ImportRecord_normalized_url_check','ImportRecord_protected_url_check'))=4
