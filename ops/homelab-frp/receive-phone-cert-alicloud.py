#!/usr/bin/env python3
"""Restricted SSH forced command for the VM104 phone API certificate deploy key."""
...
BASE = Path('/opt/docker/npm/data/nginx/custom/nomad/tls')
MAX_INPUT = 100_000


def run(*args: str) -> bytes:
    return subprocess.check_output(args, stderr=subprocess.DEVNULL, timeout=45)
...
def main() -> int:
    ...
    if run('openssl', 'x509', '-in', str(leaf_path), '-pubkey', '-noout') != \
       run('openssl', 'pkey', '-in', str(key_path), '-pubout'):
        return 68
    current = BASE / 'current'
    old_target = os.readlink(current) if current.is_symlink() else None
    target = str(Path('releases') / digest)
    ...
    try:
        run('sudo', '-n', '/usr/bin/docker', 'exec', 'npm', 'nginx', '-t')
        run('sudo', '-n', '/usr/bin/docker', 'exec', 'npm', 'nginx', '-s', 'reload')
