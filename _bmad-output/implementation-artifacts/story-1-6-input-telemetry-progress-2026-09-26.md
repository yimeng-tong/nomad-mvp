...
复核FIFO时发现遮挡超过原两秒观察上限会使窗口永远无法开始；已将Dock观察改为取消即停的250ms低频持续检查，结果Sheet仍保留两秒有界窗口。实际App浏览器遮挡超过2.15秒后再移开，才确认`ingest_presented`；本地283移动+5配置、完整构建/typed lint及108源码指纹复验通过。首提交566d96b已推送；本修补待追加推送/CI，不把前一轮CI当最终源码验收。

最终修补提交`d02d0b0`已推送；CI`36221667847`两job成功。新增输入遥测产物108源码匹配，四ZIP合计999文件CRC通过；三引擎页面129 expected/0 unexpected/0 flaky、33视觉场景、313源码匹配。精确产物核验和未关闭项见`story-1-6-input-telemetry-acceptance-2026-09-26.md`。上段“待追加推送/CI”为执行过程记录，以本段为当前结论。
