...
- [ ] T0 固定实际能力、运行环境与身份边界（AC1–3、11–14；FR14 / login-attribution）
  ...
  - [ ] 核验PNVS短信系统签名/模板、图形H5 appId/appKey、友盟平台注册类型/宿主与事件入口、homelab目标及HTTPS/可信Origin；适用Apple/微信路径按已批准启用方式核验。仅登记受控配置引用，不把密钥/真实号码放入Git或日志。
...
- 配置校验组件已支持用户选定PNVS短信认证＋图形H5；25项测试通过，保留真实模式/替身、TLS和隐私边界。只读auth-preflight新增独立资源存在性摘要，模板及脚本命令已更新；尚未接入业务启动/路由，现存生产认证风险未宣称已解决。
- 已定位根目录阿里云AK配置，U-App标识保存于独立私有配置。实际预检只报缺`ALIYUN_PNVS_SIGN_NAME`、`ALIYUN_PNVS_CAPTCHA_APP_ID`、`ALIYUN_PNVS_CAPTCHA_APP_KEY`；平台注册类型、homelab目标及真实服务/HTTPS仍待核实。当前证据：`_bmad-output/implementation-artifacts/research/story-1-0-resource-alignment-2026-09-18.md`；9月17报告保留为历史快照。
- 按dev-story缺少必要配置的HALT条件仍停在T0；矩阵登记和主路径选定已完成，T0整项与后续任务未关闭。用户已有本地实施授权继续有效；没有云API、短信发送、部署或数据操作。
