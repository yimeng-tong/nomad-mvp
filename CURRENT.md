...
2026-09-27 手机API域名网络切片已验收：`nomad-test.yinianyunqi.top` A→阿里云上海`47.101.189.96`，公网受信HTTPS `/api/*`→NPM→云端仅网桥FRP25245→VM104隔离`nomad-phone.service`回环43105。新`nomad_phone`数据库九迁移、Redis DB5、staging真实认证模式/原生audience/精确Origin与安全Cookie；原staging、development43104及旧OpenList链路保留。外网TLS验证0、健康200、HTTP→HTTPS301、匿名导入/SSE401、错误audience/Origin/Cookie403及CORS反例通过，服务/隧道重启恢复；证书DNS-01签发并启用每日续期。开发身份`dev.nomad.mvp`下手机Web构建、双平台Capacitor sync/`native:verify`通过，但WSL缺Android JDK/SDK和Xcode，无原生编译/安装证明。NPM重载窗口出现未归因短暂TLS握手失败，稳定后50/50通过，后续重载须监测。`/api/auth/config`因真实隐私政策/用户协议URL缺失仍`unavailable`，未测真实登录/认证SSE/短信。用户本次明确不要求双端实机作为域名网络验收条件；Story1.8和APP-HOST-01仍in-progress，真机与真实XHS/owner认定/异机灾备等其他门槛不由此关闭。详见`docs/ops/phone-api-domain-2026-09-27.md`和`evidence/story-1-8-homelab-2026-09-27/phone-domain-acceptance.json`。

前阶段FRP私有隧道记录：选用正在运行的VM104，安装独立`nomad-frpc.service`；阿里云上海另建`nomad-frps.service`，专用令牌与私有CA由受限服务器文件持有，客户端校验证书身份。VM104出站7001、云端仅网桥`172.17.0.1:25245`→当时VM104开发回环43104；私有健康200、匿名导入记录401、NPM容器可达，两个新服务分别重启后自动恢复，旧Docker frps7000/OpenList25244未改。公网直连25245超时；此时尚无公网HTTPS。之后已切换到上述独立手机后端43105。原始证据`evidence/story-1-8-homelab-2026-09-27/frp-acceptance.json`。
