...
- Story 2.0/2.1 and 1.1–1.5 retain historical done. Legacy 2.2 maps only to current 3.1, which
  ...
  The next preparation is 1.0 (backlog); no create-story/dev is authorized by SP.
...
- SP now tracks all 60 formal stories with explicit stable keys. The 53 current targets, including
  1.0 and migrated 3.1, have no new implementation story files yet; later authorized create-story
  must prepare the current contract and transfer its bound engineering conditions.
...
Current status and authorization remain unchanged:1.0 backlog is next;3.1 remains paused;
no create-story/dev, real services, deployment or data operations were authorized by this recheck.
