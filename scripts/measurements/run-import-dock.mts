...
if (!['matrix', 'fixed-single', 'deadline-cutoff'].includes(profile)) throw new Error('MEASUREMENT_PROFILE_INVALID');
...
const sources = ['scripts/measurements/run-import-dock.mts', 'scripts/measurements/tsconfig.json', 'packages/types/package.json', 'pnpm-lock.yaml', 'apps/mobile/vite.config.ts', 'packages/types/src/import-measurements.ts', 'apps/mobile/scripts/import-measurement-harness.tsx', 'apps/mobile/src/home/dock-controller.ts', 'apps/mobile/src/home/dock-model.ts', 'apps/mobile/src/home/HomeImportDock.tsx', 'apps/mobile/src/home/operation-journal.ts'];
const hashes = async () => Object.fromEntries(await Promise.all(sources.map(async (file) => [file, createHash('sha256').update(await readFile(resolve(root, file))).digest('hex')])));
...
try {
  let ready = false;
  for (let i = 0; i < 50; i++) { if (child.exitCode !== null) throw new Error('MEASUREMENT_SERVER_EXITED'); try { if (await (await fetch(origin + '/__measurement_identity')).text() === nonce) { ready = true; break; } } catch {} await new Promise((r) => setTimeout(r, 100)); }
  ...
  const dataset = await page.evaluate(async (selected) => {
    ...
    const { runImportMeasurement } = await import(/* @vite-ignore */ '/scripts/' + 'import-measurement-harness.tsx');
