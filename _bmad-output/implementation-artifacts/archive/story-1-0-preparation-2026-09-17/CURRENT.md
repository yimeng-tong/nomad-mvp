...
execution_phase: planning-handoff
...
2026-09-17已再次核对PRD/Epic承接；最新复验见 `sp-revalidation-2026-09-17.md`，逐需求/任务绑定见 `sprint-delivery-contract-2026-09-17.yaml`（均在implementation-artifacts）。

## Read in This Order

1. `AGENTS.md`、`_bmad-output/project-context.md`。
2. `_bmad-output/implementation-artifacts/sprint-status.yaml`：当前唯一执行状态。
3. 同目录 `sprint-delivery-contract-2026-09-17.yaml` 与 `sp-revalidation-2026-09-17.md`；再读原迁移.md/.yaml核对历史。
...
后续获得相应授权时，按sprint的`next_story_to_prepare`准备首个尚无当前合同的目标；包含继承
in-progress而contract_ready=false的3.1，不只扫描backlog。3.1准备后仍保留in-progress，
当前合同Status同步，以contract_ready=true记录准备完成；真实前置和迁移审计满足前不解除暂停。
旧2.2历史文件永不派发。准备好的其他Story正常走ready-for-dev→in-progress→review→done。

每张新合同需带源epics中的完整As a/I want/So that、Requirements/GWT、原型及实际证据要求；
同时纳入交付映射的FR/NFR、七项适用工程条件和source_obligations。特别保留1.0/1.6的既有
U-Link/U-App/归因承接及2.7地图级版权要求；真实适用性与服务证据未验证前不能报完成。
迁移manifest内authorization/execution_state是9月15日快照；当前授权与逐Story条件进展由
sprint-status维护。文件记录已有用户授权，不自行授予权限；本轮仍止于SP复核。
