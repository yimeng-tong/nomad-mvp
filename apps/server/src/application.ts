...
import ingestRoutes, { stopIngestWorker } from './routes/ingest.js';
...
import { assertAuthRuntimeImplemented } from './auth/runtime-boundary.js';
import type { PersistentAuthService } from './auth/service.js';
import { AuthFault } from './auth/errors.js';
import { getPrisma } from './db/prisma.js';

export async function buildApplication(authConfig: AuthRuntimeConfig, service?: PersistentAuthService) {
  assertAuthRuntimeImplemented(authConfig);
  resolveIngestExecutionPolicy(process.env);
  if (authConfig.provider.kind !== 'fixture' && !service) throw new AuthFault('AUTH_IMPLEMENTATION_UNAVAILABLE', 503);
  sentryInit();
  const app = Fastify({
    logger: {
      redact: ['req.headers.authorization', 'req.headers.cookie', 'req.headers["x-nomad-login-binding"]', 'res.headers["set-cookie"]'],
      serializers: { req: (req) => ({ method: req.method, route: req.routeOptions?.url }) },
    },
  ...
  await app.register(rateLimit, {
    ...
    keyGenerator: (req) => service ? req.ip : `${req.headers['x-device-id'] ?? 'nodev'}:${req.ip}`
  });

  await app.register((fastifySSE as any));
  await app.register(helmet, { contentSecurityPolicy: false });
  await app.register(cors, { origin: authConfig.allowedOrigins, credentials: true });
  ...
  await app.register(authPlugin, { service });
  if (!service) await app.register(queuesPlugin);
  ...
  app.get('/health', async () => {
    if (service) await service.repository.ready();
    return { status: 'ok' };
  });
  app.setErrorHandler((error, req, reply) => {
    if (error instanceof AuthFault) return reply.sendError(error.code, error.code, error.status, error.retriable, error.details);
    const supplied = typeof error === 'object' && error !== null && 'statusCode' in error ? error.statusCode : undefined;
    const status = typeof supplied === 'number' && [400,401,403,404,409,413,429].includes(supplied) ? supplied : 500;
    app.log.error({ code: 'REQUEST_FAILED', traceId: req.traceId }, 'request failed');
    return reply.sendError(status === 429 ? 'REQUEST_RATE_LIMITED' : status === 400 ? 'PARAMS_INVALID' : 'REQUEST_FAILED', 'Request could not be completed', status, status === 500);
  });
  app.addHook('onClose', async () => { await stopIngestWorker(app);await getPrisma()?.$disconnect(); });
  ...
  app.post<{ Body: AiFillRequest, Reply: AiFillResponse | any }>('/plan/ai-fill', { config: { rateLimit: { max: 10, timeWindow: 60 * 60 * 1000 } } }, async (req, reply) => {
    if (service) return reply.sendError('FEATURE_NOT_AVAILABLE', 'This capability is not available yet', 503, false);
