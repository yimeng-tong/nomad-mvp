...
- [x] T0 核对资源、写入责任与工具链（AC1/8；APP-BUILD-01、app-build-delivery）
  ...
  - [x] 固定同一React/Vite及Capacitor官方版本、iOS16构建目标和实际WebView下限；记录必要自主决定。
...
- [x] T1 配置/构建边界与最小依赖（AC1/3/8；NFR3/NFR25）
  ...
  - [x] 为无效/误用配置写负面测试；保留Web build和现有Vitest配置，显式iOS16/Safari16及Android内核目标。
...
- [x] T3 生成双端工程并建立构建说明（AC1/3/8；APP-BUILD-01）
  - [x] 使用官方Android/SPM iOS模板生成原生源码，保留UIScene/SceneDelegate；minSdk29和deployment16.0。开发候选标识与正式资源登记分开，未知正式条件不写成完成。
...
- [ ] T6 真机与关闭证据（AC1–8；APP-HOST-01、app-build-delivery）
  ...
  - [ ] iOS16与最低/当前支持设备、低内存/无GMS分支有可验证结果；未覆盖则不声明已支持。
...
依据[官方版本及资源研究](research/story-9-1-capacitor-host-research-2026-09-19.md)：官方8.5.2主体与app8.1.1/keyboard8.0.5/browser8.0.4；JDK21/AGP8.13/Gradle8.14.3/SDK36，产品minSdk29；iOS16与Xcode26+、SPM、UIScene。WSL生成/同步需实际运行确认，Mac编译/签名不能替代。Vite默认iOS16.4，显式target修正仍需Web API/CSS真机核验。
