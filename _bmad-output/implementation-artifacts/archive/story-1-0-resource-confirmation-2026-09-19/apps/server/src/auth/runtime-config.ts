...
type CaptchaConfiguration =
  ...
  | { kind: 'aliyun-pnvs'; mode: 'risk' | 'always'; appId: string; appKey: string }
  | {
...
export function readAuthRuntimeConfig(env: AuthEnvironment): AuthRuntimeConfig {
  ...
  const captcha: CaptchaConfiguration = rawProvider === 'fixture'
    ...
    ? {
      kind: 'aliyun-pnvs', mode: captchaMode === 'always' ? 'always' : 'risk',
...
export function summarizeAuthResourceInputs(env: AuthEnvironment) {
  ...
  return {
    ...
    pnvsGraphic: { appIdPresent: present('ALIYUN_PNVS_CAPTCHA_APP_ID'), appKeyPresent: present('ALIYUN_PNVS_CAPTCHA_APP_KEY') },
    attribution: {
