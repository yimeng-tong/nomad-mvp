...
export function checkSprintDelivery({ current, sprint, migration, targets, byId, read, exists, scope }) {
  ...
  const expectedObligations = { 'login-attribution': ['1.0'], 'input-attribution': ['1.6'], 'analytics-consolidation': ['8.1'], 'map-copyright': ['2.7'], ...(scope ? { 'app-build-delivery': ['9.1', '9.2'] } : {}), ...(scope?.additionalObligations ?? {}) };
  ...
  for (const [name, item] of Object.entries(delivery.source_obligations)) {
    assert.deepEqual(item.story_keys, expectedObligations[name].map((id) => byId[id]), `Wrong first-use owner: ${name}`);
  ...
  if (scope) {
    assert.equal(delivery.scope_revision, scope.id);
    for (const id of ['9.1', '9.2', ...Object.keys(scope.additions)]) {
      assert.ok(delivery.requirements.FR52.delivery_story_keys.includes(byId[id]), `FR52 lost App delivery slice ${id}`);
      assert.ok(delivery.requirements.NFR25.delivery_story_keys.includes(byId[id]), `NFR25 lost native evidence slice ${id}`);
    }
  }
