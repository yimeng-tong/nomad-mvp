...
第二候选ea47462/run36150885851的24项、源/lock/config/环境/PNG hash在CI和下载后均验证通过。9张变化图片已重新审阅，15张与第一轮已审阅内容字节一致；V03另看原分辨率并核对三引擎geometry，文字在card内部。已显式纳入24张初始基线及approval/review记录，尚需常规零差异比较。T6本机三种实际页面故障（外置私有层、无视取消的迟到HTTP回调、回执查询时重复原POST）均目标失败且各有trace，前后真实流程控制共6项通过。已接完整90项矩阵/源码当前run检查、像素故障与配置负例到CI，等待实际远端执行，不预标T5/T6完成。


f53d2a8/run36152353310正常三引擎90/90、0skip、24张零阈值视觉比较及当前源码完整性检查通过。三种私有层/迟到回调/重复写反例均如期失败；CTA也真实像素失败，但后续产物检查把expected attachment名称误当path后缀，导致job失败。已修正导出并保留原失败，T6不先勾选。T4/T5各自实现与实证完成；T7文档/根命令已接，保持所有旧链和原探针职责。

原auth-browser六项与原durable-dock-pg六项已定向重跑：Chromium153、PG18.6/SSE/IDB、三进程一次SIGKILL、零新导入POST。只作输出/模块路径重定位；原28份历史文件逐hash不变。原PG脚本两次setup失败因本轮新隔离库继承Asia/Shanghai使合成Session时间比较过期；仅把该新库设UTC后原断言通过，未改服务或生产库。失败与通过均封存在legacy目录。临时PG/Redis及5187/5193回环服务已关闭，库文件保留；其他旧浏览器/压力/生产备份责任不计本次重跑通过。

三层完整CR合并8项patch已修补并定向复核，无新增明确问题。本机23×2流程、5网络故障/2控制、4个功能故障/6控制、4个真实下载报告完整性负例及242+5/18工作台/typed lint/native隔离/handoff通过。R4/R8改为v2完整来源/编译bytes/实际PNG证明；矩阵增至93（B22 OTP合同），尚须最终canonical/全部负例CI及下载核验，不标Story done。

2026-09-26关闭：完整CI36157432347通过，93项v2、全部反例、777下载文件/12trace及24actual验证完成。9.5两条件verified、8项CR修补验证闭环，进入done交接；下一9.3仅按当前源临近准备，其他Story真实门槛和3.1暂停不变。
