...
pnpm -F nomad-mobile run native:sync android
pnpm -F nomad-mobile run native:sync ios
...
Android minimum OS is API29, with WebView111+ for the current bundle. Older engines use the bundled recovery page. Vite explicitly targets Safari/iOS16.4 and Firefox128 (Chromium/Edge111); syntax targets do not polyfill every Web API or prove device compatibility. Shared inset tokens are consumed by each existing page/footer, not padded again on html/root.
...
## Approved UI compatibility amendment (2026-09-20)

Current product minimum: iOS16.4+, Android10+/WebView111+, WebSafari16.4+/Firefox128+/Chromium111+/Edge111+. Web JS/CSS, all project/App deployment configurations and App SPM must agree; native:verify rejects mixed old targets. Dependency installation and shared components remain separate9.3–9.7 work. Prior16.0 build evidence and APK hashes are historical, not proof of the new minimum or real-device operation.

Use the workspace native:sync script: Capacitor8 regenerates App SPM from the major Xcode version, then the scoped wrapper retains16.4 without rewriting dependencies. Unexpected platform declarations fail. Run native:verify after sync; direct cap sync alone does not certify the product minimum.
