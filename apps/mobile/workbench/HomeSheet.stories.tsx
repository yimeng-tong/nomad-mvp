...
function SheetExample({ largeText = false, state = 'normal' }: { largeText?: boolean; state?: 'normal' | 'empty' | 'partial' | 'unknown' | 'long' }) {
  ...
  return <TextScale enabled={largeText}><main className="workbench-stage">
    ...
    {open ? <HomeSheet label="灵感说明" onClose={() => setOpen(false)}>
      ...
      <button type="button" onClick={() => setOpen(false)}>返回</button>
...
export const KeyboardAndFocus: Story = {
  ...
  play: async ({ canvas, userEvent }) => {
    ...
    await expect(sheet.getByRole('button', { name: '返回' })).toHaveFocus();
    await userEvent.tab({ shift: true });
    await expect(sheet.getByRole('button', { name: '完成' })).toHaveFocus();
    await userEvent.tab();
    await expect(sheet.getByRole('button', { name: '返回' })).toHaveFocus();
...
export const LongChinese: Story = { name: '长中文', args: { state: 'long' }, play: async ({ canvas, userEvent }) => {
  ...
  await expect(canvas.getByRole('button', { name: '打开灵感说明' })).toHaveFocus();
} };
...
export const Unmount: Story = { name: '卸载 · 有效触发器焦点恢复', play: async ({ canvas, canvasElement }) => {
  ...
  try {
    root.render(<HomeSheet label="卸载测试" onClose={() => root.unmount()}><button type="button">临时操作</button></HomeSheet>);
    await waitFor(() => expect(canvas.getByRole('button', { name: '临时操作' })).toHaveFocus());
    root.unmount();
    await expect(trigger).toHaveFocus();
