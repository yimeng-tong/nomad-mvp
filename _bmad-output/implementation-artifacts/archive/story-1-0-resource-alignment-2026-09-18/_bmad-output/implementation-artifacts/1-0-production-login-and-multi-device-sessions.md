...
> 本文件是待实施合同。14组已批准GWT保持原文；准备就绪不表示真实服务、迁移、恢复或生产开放已经验收。
...
交付映射另明确 NFR6 的 `login-attribution`，以及 OPS-01、DB-CHANGE-01、METRICS-01/02/03。这些是同一Story的任务与条件，不增加产品Story或改写GWT。
...
所有业务任务尚未实施，不能从历史1.1/1.2或本次文档准备继承勾选。

- [ ] T0 固定实际能力、运行环境与身份边界（AC1–3、11–14；FR14 / login-attribution）
  - [x] 写出脱敏的宿主/方式/供应商矩阵：移动Web、安装PWA、桌面、微信内网页及确实已有的原生宿主分别登记；未核验资源写待核验，不断言不存在。
  - [ ] 优先复用已有获准资源。Authing是统一身份入口的主候选；极光JSMS/JVerification是可选手机号证明来源。依据实际可用租户/权益固定一条主路径，不同时引入两套用户目录，不把一键取号当成所有浏览器通用短信登录。
  - [ ] 核验真实短信、Apple Services ID/关联App、适用微信应用/回调、腾讯Web验证码、友盟宿主/事件入口、HTTPS与可信Origin；仅登记受控配置引用，不把密钥/真实号码放入Git或日志。
...
- [ ] T4 真实证明适配与受限调用（AC1–4、11–12）
  ...
  - [ ] 腾讯验证码按真实Web实例服务端验票；保存Ticket/Randstr的最小短期关联，只有适用服务端通过结果才继续，不把前端ret=0、非空token或 `trerror_` 容灾当作通过。
...
- [ ] T7 移动登录与最小桌面授权使用路径（AC1、7–8、10–14）
  - [ ] 复用现有LoginScreen/配置驱动等权入口、协议链接、冷却/错误和先 `/me` 确认的流程；接通实际captcha与本期获准第三方跳转/返回，不重建另一个应用壳。
...
- [ ] T8 `login-attribution` 首次消费闭环（FR14、NFR6；AC1、10–12、14）
  ...
  - [ ] 在实际适用宿主核验U-Link接收端与U-App事件/查询能力。H5唤起App不等于纯PWA完整归因；若需U-Web/U-Mini或新宿主，提交具体差异进入已有范围决定，不能自行替换/延期FR14或新建原生App。
...
1.0是首个新增执行准备，复用已交付1.1/1.2及领域能力；其14组GWT不依赖未来7.5清理页。1.6–1.11新增导入、完整1.7重放、3.1分钟编辑迁移、7.3完整设置、7.4副本、7.5删除清理、8.x运营页面均不混入本Story。关闭现有裸入口是本Story安全责任；未实现的fill/export只诚实受限，不在此补齐5.x。

### 现状 / 改变 / 保留

后端逐路径及完整读取清单见 [后端核验](research/story-1-0-backend-context-2026-09-17.md)。以下均为本次工作树事实，尚未修改：
...
- 新增runtime-config配置校验组件、18项测试、只读auth-preflight和空值.env.example；尚未接入业务启动/路由，不把现存生产认证风险宣称已解决。
- 本地仅有DATABASE_URL/REDIS_URL；T0实际供应商/宿主/受控配置待用户信息。资源报告：`_bmad-output/implementation-artifacts/story-1-0-runtime-preflight-2026-09-17.md`。
- 按dev-story缺少必要配置的HALT条件停在T0；未越到T1；仅T0的矩阵登记子项完成，T0整项与其余实现/实证继续未关闭。当前本地实施授权不失效。
