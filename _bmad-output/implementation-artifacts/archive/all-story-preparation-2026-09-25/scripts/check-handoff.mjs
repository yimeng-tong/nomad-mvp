...
import { APP_CONDITION_OWNERS, checkScopeAmendment, checkPreparedAppContract } from './check-capacitor-scope.mjs';
import { checkUiScopeAmendment, checkPreparedUiContract } from './check-ui-scope.mjs';
...
export function checkHandoff(root = process.cwd()) {
  ...
  const scope = (migration.schema_version === 3 ? checkUiScopeAmendment : checkScopeAmendment)({ current, sprint, migration, epicsText, read, exists });
  const expected = scope?.counts ?? { epics: 8, stories: 60, historical_stories: 7, current_target_stories: 53, gwt_scenarios: 1018, functional_requirements: 65, mvp_functional_requirements: 61, deferred_functional_requirements: 4, nonfunctional_requirements: 24 };
  const conditionOwners = { ...CONDITION_OWNERS, ...(scope ? APP_CONDITION_OWNERS : {}), ...(scope?.conditionOwners ?? {}) };
  const historicalRead = scope?.frozen ?? read;
  ...
  for (const match of readinessText.matchAll(/^\| `([^`]+)` \| `([a-f0-9]{64})` \|$/gm)) {
    assert.equal(sha256(historicalRead(match[1])), match[2], `Approved planning input changed: ${match[1]}`);
  }
  ...
  assert.deepEqual(epics, Array.from({ length: expected.epics }, (_, index) => index + 1), 'Each formal Epic appears exactly once');
  assert.equal(stories.length, expected.stories);
  assert.equal(new Set(stories.map((s) => s.id)).size, expected.stories, 'Duplicate formal Story ID');
  ...
  assert.equal(gwt, expected.gwt_scenarios);
  assert.equal(fr.length, expected.functional_requirements);
  assert.equal(new Set(fr).size, fr.length);
  assert.equal(nfr.length, expected.nonfunctional_requirements);
  assert.equal(new Set(nfr).size, nfr.length);
  assert.equal(readiness.storyCount, scope?.baseline.scope_counts.stories ?? stories.length);
  assert.equal(readiness.gwtScenarioCount, scope?.baseline.scope_counts.gwt_scenarios ?? gwt);
  assert.deepEqual(migration.scope_counts, expected);
  if (scope) {
    const targeted = frontMatter(read(scope.readiness_report), scope.readiness_report);
    assert.equal(targeted.scope_revision, scope.id);
    assert.equal(targeted.status, 'passed-targeted-revalidation', 'Complete targeted readiness before dispatch');
    assert.equal(targeted.readinessScope, scope.readinessScope ?? 'capacitor-local-implementation');
    assert.equal(targeted.productionReady, false);
    assert.equal(targeted.storyCount, stories.length);
    assert.equal(targeted.gwtScenarioCount, gwt);
    assert.deepEqual(targeted.openFindings, []);
    assert.ok(targeted.runtimeGates.length, 'Real platform gates remain distinct from planning readiness');
  }
  ...
  assert.deepEqual(Object.keys(status), expectedKeys, `Sprint must contain exactly ${expected.epics} Epics, ${expected.stories} Stories and ${expected.epics} retrospectives in order`);
  ...
  assert.equal(targets.length, expected.current_target_stories);
  equalSet(Object.keys(migration.implementation_conditions), Object.keys(conditionOwners), 'Keep all seven engineering conditions and approved scoped additions, without extra product Stories');
  ...
  const delivery = checkSprintDelivery({ current, sprint, migration, targets, byId, read, exists, scope });
  if (scope) equalSet(migration.preparation_order, targets, 'Approved preparation order must cover exactly the current targets');
  else assert.deepEqual(migration.preparation_order, targets, 'Preparation order skips historical completed work');
  const superseded = {
  ...
  for (const key of targets) {
    ...
    if (status[key] !== 'backlog' && !inheritedUnprepared) {
      ...
      if (scope) checkPreparedAppContract({ key, id: catalog[key].story_id, storyText, source: scope.blocks[catalog[key].story_id], status: status[key], sprint, read, exists, nativeConditionBound: migration.story_condition_bindings[key].includes('APP-HOST-01') });
      if (scope?.conditionOwners) checkPreparedUiContract({
        key, id: catalog[key].story_id, storyText, status: status[key], sprint,
        conditions: migration.story_condition_bindings[key], read, exists,
        sourceHash: catalog[key].contract_sha256,
        source: scope.blocks[catalog[key].story_id],
        obligations: Object.keys(scope.additionalObligations).filter((name) => delivery.source_obligations[name].story_keys.includes(key)),
      });
      if (key === editKey) {
        const meta = frontMatter(storyText, file);
  ...
  for (const [id, condition] of Object.entries(conditions)) {
    assert.equal(condition.primary_story, byId[conditionOwners[id]], `Incorrect first owner for ${id}`);
    ...
    assert.equal(condition.source, scope?.conditionOwners?.[id] ? scope.conditionSource : `${PLANNING}/${id.startsWith('APP-') ? 'app-implementation-prerequisites-2026-09-19.md' : 'implementation-prerequisites-2026-09-15.md'}`);
  ...
  assert.equal(report.workflow, scope ? 'bmad-correct-course' : 'bmad-sprint-planning');
