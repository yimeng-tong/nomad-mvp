...
export function checkHandoff(root = process.cwd()) {
  ...
  assert.equal(current.working_branch, BRANCH, 'Keep the inherited Git history while preparing the next story');
  ...
  assert.deepEqual(sprint.retrospective_scope, migration.retrospective_scope);
  ...
  equalSet(Object.keys(conditions), Object.keys(CONDITION_OWNERS), 'Keep all seven engineering conditions, without extra product Stories');
  ...
  for (const [id, condition] of Object.entries(conditions)) {
    ...
    assert.equal(condition.execution_state, 'not-started', 'SP assignment is not operational evidence');
  ...
  assert.deepEqual(sprint.authorization, migration.authorization);
  assert.deepEqual(sprint.authorization, { sprint_planning: true, create_story: false, implementation: false, real_services: false, deployment: false, data_operations: false });
  assert.equal(current.sprint_planning_authorized, true);
  assert.equal(current.create_story_authorized, false);
  assert.equal(current.implementation_authorized, false);
  assert.equal(current.current_story, byId['1.0'], 'First new preparation must be production authentication');
  assert.equal(sprint.next_story_to_prepare, current.current_story);
  assert.equal(current.current_epic, 1);
  assert.equal(current.current_story_status, status[current.current_story]);
  assert.equal(current.current_story_status, 'backlog', 'SP must not automatically prepare the next Story');
  assert.equal(current.current_story_file, null);
  assert.equal(current.current_story_spec, sprint.epics_source);
  assert.equal(current.next_bmad_action, 'bmad-create-story');
  ...
  assert.equal(current.last_completed_story, '2-1-generate-day-skeleton-with-quick-and-hq-planning');
