...
function safeTitle(value: string | null | undefined) { return value?.replace(/[\u0000-\u001f\u007f-\u009f\u202a-\u202e\u2066-\u2069]/g, '').slice(0, 120) || '分享内容'; }
...
export function HomeImportDock({ controller, selectedCount, onPlan, onView, notice, onHeight, onRecognizedPlan, active = true }: {
  ...
  useEffect(() => { if (auth.phase === 'authenticated' && inbox.recoverySignal) void controller.restore(true); }, [controller, auth.phase, inbox.recoverySignal]);
  useEffect(() => {
  ...
  }, [controller, state.presenting?.key, state.visible]);
  useLayoutEffect(() => {
  ...
  return <footer ref={ref} className={`home-import-dock ${state.expanded ? 'is-expanded' : 'is-compact'}`} aria-label="导入与旅行输入">
    ...
    {completion ? <div className="dock-completion">
      ...
      <button type="button" onClick={() => onView(completion.jobId)}>查看</button>
        ...
        return <section key={batch.id} aria-label={`导入批次 ${state.batches.indexOf(batch) + 1}`}>
          ...
          <ol className="dock-items">{entries.map((entry) => <li key={entry.id}>
            ...
            <div className="dock-item-actions">
              {entry.acceptance === 'rejected' ? <button type="button" onClick={() => { void controller.restoreEntry(entry.id); input.current?.focus(); }}>放回输入框</button> : null}
              {entry.acceptance === 'unknown' || state.uncertainRetries.includes(entry.id) ? <button type="button" disabled={state.busy.includes(entry.id)} onClick={() => void controller.recover(entry.id)}>确认{state.uncertainRetries.includes(entry.id) ? '重试' : '受理'}结果</button>
                : entry.snapshot?.actions.retry ? <button type="button" disabled={state.busy.includes(entry.id)} onClick={() => void controller.retry(entry.id)}>重试这条导入</button> : null}
              {entry.connectionLost ? <button type="button" onClick={() => void controller.reconcile()}>重新确认状态</button> : null}
              {entry.jobId && entry.snapshot?.actions.view && entry.snapshot.result ? <button type="button" onClick={() => onView(entry.jobId!)}>查看已保存内容</button> : null}
    ...
    {state.journalError ? <button type="button" onClick={() => void controller.restore(true)}>重新读取记录</button> : null}
    ...
    <div className="dock-composer">
      ...
      <button className="dock-send" type="button" aria-label={state.input.trim() ? '发送' : '添加'} disabled={state.parsing}
        onClick={() => { inputInbox.clearNotice(); if (state.input.trim()) void controller.submit(); else { input.current?.focus(); controller.setNotice('可长按输入框粘贴，或直接输入旅行想法。'); } }}>
    ...
    <div className="basket-bar"><span>已选 {selectedCount}</span>
      <button type="button" aria-label="从剪贴板粘贴" disabled={state.pasting} onClick={() => { inputInbox.clearNotice(); input.current?.focus(); void controller.paste(readClipboardText); }}>{state.pasting ? '正在读取' : '粘贴'}</button>
