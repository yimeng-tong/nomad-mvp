...
export function checkHandoff(root = process.cwd()) {
  ...
  const scope = checkScopeAmendment({ current, sprint, migration, epicsText, read, exists });
  ...
  const conditionOwners = { ...CONDITION_OWNERS, ...(scope ? APP_CONDITION_OWNERS : {}) };
  ...
  if (scope) {
    ...
    assert.equal(targeted.readinessScope, 'capacitor-local-implementation');
  ...
  for (const [id, condition] of Object.entries(conditions)) {
    ...
    assert.equal(condition.source, `${PLANNING}/${id.startsWith('APP-') ? 'app-implementation-prerequisites-2026-09-19.md' : 'implementation-prerequisites-2026-09-15.md'}`);
