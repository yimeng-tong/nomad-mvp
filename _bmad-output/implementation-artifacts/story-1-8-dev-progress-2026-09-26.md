...
2026-09-27的旧数据只读审计：在VM104现有共享PG的已验证物理基础备份后，保留`nomad_staging`私有dump，建立隔离副本并只对副本应用缺少的七个迁移，使其与当前九迁移源版本匹配。源/副本的User2、IngestJob17、Inspiration17行数一致，源库迁移数仍2；重复读事务确认只读，并在内存分析后只输出汇总：17个旧job及两处旧明文URL列各17条、同/跨owner等价组与活动record碰撞均0，但17个job的旧`auth_version`为空，需要逐条owner权威证据；candidate0、blocked17，**未运行自动回填**，旧hash、回执、真实行未改。私有dump/副本与迁移日志留VM104，回填映射与前向恢复方案见`docs/ops/import-record-url-protection.md`。

同日homelab对象与恢复：新VM108独立32GiB数据盘、20GiB名义卷上限，SeaweedFS内环8334/Nginx内网8333、应用/备份分桶凭据及UFW源限制。`objects.yinianyunqi.top`的AliDNS DNS-01公开可信证书签发/每日续期、VM104验证TLS及签名Put/Get/Delete、匿名与跨桶拒绝、服务重启及客机重启后自动恢复均通过。服务端新增受限真实字节Put/Get与owner+活动Inspiration的私有Asset读取路由；当前`rehostMedia`仍为合成键，未启用真实XHS媒体获取，删除关联对象的物理清理未执行。VM104主PG已启用独立桶加密pgBackRest、每日全量/15分钟WAL归档检查；真实备份+WAL在私有socket恢复到指定时点，合成源marker1/2而恢复仅marker1，源staging与development健康。首次ACME传播、SeaweedFS端口、恢复实例连接数及`a058c68` typed-lint失败均保留；修复提交`4b7e47c`完整CI`36254458183`两job通过，主PG28项/旧record升级七项，且已部署到独立development九迁移；Node适配器实桶合成字节和匿名401通过。脱敏证据`evidence/story-1-8-homelab-2026-09-27/validation.json`。同PVE宿主备份/媒体、历史密钥异机保管、真实负载RPO、XHS短链/媒体、owner认定及双端真机/原生API HTTPS均未验收，T1–T5与整张Story保持未勾选，不提前准备1.9。

同日三层代码审查提出九项可修问题，均已进入修补工作树：媒体下载期间删除与长事务、证书账号越权及多行SSH公钥注入、跨桶读取负例缺失、旧库副本只核迁移名称、PITR成功先于停止、pgBackRest配置部分修改回滚、返回对象未核hash。VM108已实际移除证书账号的S3配置读取权，恶意双行合成公钥被拒且authorized_keys未变；VM104重部署原证书及签名HTTPS探针仍通过。VM108以两桶合成对象实测双方GET/HEAD/LIST/DELETE/PUT跨桶全部拒绝。VM104只读核对源两条/副本九条迁移checksum和Story1.8目标列/索引/约束通过，源/副本计数未变；全库datamodel diff未做。原PITR恢复实例实际停止且源仍运行。代码本地workspace build、typed lint、四个定向服务端测试、七个Postgres ops单测和handoff通过；下载期间删除的新增隔离PG探针及对象篡改字节的VM104 Node实桶探针待新提交/部署复验。详见`evidence/story-1-8-homelab-2026-09-27/review-validation.json`。不将已修脚本静态检查当成新PITR实操证明，Story和工程条件仍为in-progress。

后续提交`6348743`完整CI`36257939023`两job成功；下载Schema、Upgrade、Audit产物的HEAD均匹配，PG29项含下载期间删除后拒绝媒体、升级七项及只读审计迁移checksum/目标结构守卫全通过。精确提交部署VM104独立development，九迁移无新增；Node实桶合成PNG正常读取、同长度篡改拒绝且对象已删除。原staging仍健康/17 job，发展环境健康、匿名Asset401，归档WAL18/失败0；VM108桶双向权限负例复测通过。用户补充FRP后查到阿里云上海frps0.65.0、现有`cloud.yinianyunqi.top`→内侧25244的旧OpenList路由，最后`alist_tcp`成功注册于8月1日；目前无在线FRP客户端/代理，运行中的homelab来宾和46份homelab Markdown未定位frpc。它不是当前Nomad HTTPS；见`docs/ops/frp-investigation-2026-09-27.md`和`evidence/story-1-8-homelab-2026-09-27/review-acceptance.json`。Story、1.9、3.1状态不变，真实XHS、旧owner认定、异机恢复、正式API和双端真机继续单独开放。
