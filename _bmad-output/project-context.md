...
## Current Approved UI Scope (2026-09-20; supersedes earlier platform and queue summaries)

User approved the complete UI proposal/appendix including Firefox128+/WebSafari16.4+ and separate Query9.6/Router9.7. Nomad uses shadcn/ui + Base UI + Tailwind4 via its own shared primitives/composites; existing branding, business/auth/journal/cursor ownership remains. iOS minimum16.4, Android10+/WebView111+; Chromium/Edge111+. Current source/catalog/delivery contain9 Epics/67 Stories/1089 GWT/66 FR/25 NFR. Story9.4 completed implementation, independent review and full CI on2026-09-25 (6344a53/run36129441895); complete downloaded artifacts were verified. Story9.5 completed implementation and independent CR on2026-09-26: 85eedb6/run36157432347,93 actual canonical cases and777 downloaded artifacts, with only its CODE-QUALITY-01/UI-BROWSER-01 conditions verified;9.3 local UI slice passed independent CR and full CI36184294356 on aeadc1e:129 product cases/33 visuals,6 shared-component cases across3 engines,32 workbench cases and88 typed lint files. Its four UI conditions/local prerequisite are verified;9.3 stays in-progress for real native T9/APP-HOST-01.9.6/9.7 remain backlog. Next action is the approved in-progress integration and remaining code work before just-in-time1.8. The user explicitly resumed development on2026-09-25; contract readiness alone still does not waive dependencies. Explicit UI quality conditions and per-Story evidence remain.

Decision: `_bmad-output/planning-artifacts/ui-foundation-scope-decision-2026-09-20.md`. Current paths come from CURRENT.md, not older dated paragraphs. Prepared1.0/1.6/1.7/9.1 source fingerprints/Tasks are synchronized; historical7done and3.1pause remain. 9.4 done with scoped CODE-QUALITY-01/UI-WORKBENCH-01 proof;9.5 done with its own CODE-QUALITY-01/UI-BROWSER-01 proof; next preparation1.8 after9.3 CS/VS; Query/Router before2.3,9.2last. The later explicit user direction recorded in _bmad-output/implementation-artifacts/sprint-execution-resume-2026-09-25.md removes stop_after_story1.7. Execute9.4→9.5→9.3 with just-in-time CS/VS, then recheck in-progress remaining slices. Real native/operational gates and3.1pause remain open.

## Prior Approved App Scope (2026-09-19; historical baseline for the UI successor)

The user approved `_bmad-output/planning-artifacts/capacitor-scope-decision-2026-09-19.md`: preserve Web/PWA, add Capacitor Android APK and iOS TestFlight; Android10+/iOS16+ phones. Current inventory is9 Epics/62 Stories/1052 GWT/66 FR/25 NFR. New9.1 delivers an installable existing login/terms host before native auth closure;9.2 delivers final test distribution after business evidence. Each existing Story owns its native capability, permissions, recovery and real-device proof. New source/contract fingerprints are in the current9月19catalog/delivery.

Continuous current-MVP preparation/development/validation/review is explicitly authorized, with autonomous decisions recorded. Do not request routine next-Story authorization again. Missing essential keys/resources block only the dependent slice; keep independent work moving, preserve actual prerequisites and never manufacture done. Historical7 completed Stories,3.1 inherited pause and all deferred scope remain. Earlier “no new native App” notes describe the previous boundary, now superseded by this decision.

Coordinate the existing1.0 task and9.1 task using Sprint active_workstreams; preserve shared code and current evidence. Primary CURRENT may remain1.0 while9.1 progresses separately. As explicitly updated on2026-09-25, WSL and macOS support Git-synchronized development; Mac may perform component and iOS work. Keep pinned Node/pnpm, server secrets on homelab, and one shared-file writer. Windows-native Node/pnpm remains excluded. APP-BUILD-01/APP-HOST-01/APP-DISTRIBUTE-01 are per-Story conditions; real installs/signing/TestFlight and provider evidence remain runtime gates.
...
- The repo is a pnpm monorepo with shared React/Vite Web/Capacitor mobile, Fastify/TypeScript ESM server, Prisma and
...
- Story 2.0/2.1 and 1.1–1.5 retain historical done. Legacy 2.2 maps only to current 3.1, which
  ...
  The user approved starting 1.0 on 2026-09-17 and then authorized dev-story. It is in-progress;
  on2026-09-19 resource configuration passed and the existing homelab VM104 was started under
  explicit user authorization. Authentication/session implementation and live proof remain open.
...
- S11 exports the exact current revision as `导出图片`: base itinerary is always available, current
  ...
  one logical batch action. It never uses ZIP or detaches a DayExcursion from its host section; the Web/PWA branch does not
  promise direct photo-library write, while the approved App branch requires actual save receipts; system sharing remains capability-gated and city-image ordered.
...
- The amended Sprint tracks62 formal stories with stable keys. Prepared contracts and remaining
  preparation are determined from the current catalog; migrated3.1 retains its separate pause/audit. Each contract carries its source fingerprint and bound engineering conditions.
...
## Story 1.0 Resource Confirmation and Homelab Authorization (2026-09-19)

The user supplied all three PNVS graphic profiles and both Android/iPhone U-App resources,
confirmed PNVS is enabled, and provided approved system SMS signs/templates. Current browser
configuration explicitly selects the `nomadh5` scheme. Use the approved sign恒创联众 and login
template100001; no PNVS-specific SMS key is missing because SMS uses the Alibaba AccessKey pair.
Private credentials remain in ignored0600 env files. Do not repeat values or copy them to source.

Current evidence is `implementation-artifacts/research/story-1-0-resource-confirmation-2026-09-19.md`.
Actual read-only preflight succeeds with configurationValidated=true and realServicesVerified=false.
The validator has27 passing tests including graphic platform separation and dual U-App inventory.
Native app keys do not establish Web/PWA U-App/U-Link integration. The pasted暂停使用 label may be
an action button; its service state is unverified, not an established disabled account.

User authorization now includes creating/reusing homelab VM/LXC for development and testing.
PVE capacity and existing VM104 identity/no-passthrough were checked; VM104 was started, guest
services and local health verified, and Windows SSH through PVE with ProxyJump succeeded.
Existing release10f940c49e2d and retained data/configuration remain; no application update, database
migration, secret deployment, SMS or provider proof was performed. OPS-01 is in-progress for this
environment preflight only; backup/restore/security gates remain open.

Original missing-resource blockers are resolved. Continue1.0 T0 runtime/host integration, preserving
all product/security/attribution obligations. Do not dispatch1.6 or resume3.1. The user explicitly
requires overall development/testing before public-cloud publication; current deployment scope is
homelab only. The dated SP authorization snapshot remains unchanged.
